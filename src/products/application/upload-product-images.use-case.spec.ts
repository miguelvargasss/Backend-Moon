...
describe('UploadProductImagesUseCase — HUMP04', () => {
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<UploadProductImagesUseCase>(
      UploadProductImagesUseCase,
    );
  });
  ...
  it('debería lanzar NotFoundException si el producto no existe', async () => {
    ...
    await expect(useCase.execute('inv', mockFile)).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('CA6 — debería subir imagen y devolverla', async () => {
    ...
    mockSupabaseService.adminClient.storage.upload.mockResolvedValue({
      error: null,
    });
  ...
  it('debería lanzar Error si la subida falla', async () => {
    ...
    await expect(useCase.execute('1', mockFile)).rejects.toThrow(
      'Error subiendo imagen: Upload Failed',
    );
  });
});
