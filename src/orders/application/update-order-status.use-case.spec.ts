...
const mockOrderItems = [
  { id: 'item-1', orderId: 'order-1', productId: 'prod-1', quantity: 2, priceAtSale: 50 },
];
...
describe('UpdateOrderStatusUseCase — HUMP08 (Gestión de Pedidos y MoonPoints)', () => {
  ...
  it('CA3 — debería lanzar NotFoundException si el pedido no existe', async () => {
    ...
    await expect(useCase.execute('order-x', 'CONFIRMADO')).rejects.toThrow(NotFoundException);
    await expect(useCase.execute('order-x', 'CONFIRMADO')).rejects.toThrow('Pedido no encontrado');
  });
  ...
  it('CA4 — debería lanzar BadRequestException si el estado nuevo no existe en BD', async () => {
    ...
    await expect(useCase.execute('order-1', 'ESTADO_FALSO')).rejects.toThrow(BadRequestException);
    await expect(useCase.execute('order-1', 'ESTADO_FALSO')).rejects.toThrow('no encontrado');
  });

  it('debería lanzar BadRequestException si el pedido ya está FINALIZADO', async () => {
    mockOrderRepository.findById.mockResolvedValue(makeOrder({ statusName: 'FINALIZADO' }));

    await expect(useCase.execute('order-1', 'ENVIADO')).rejects.toThrow(BadRequestException);
    await expect(useCase.execute('order-1', 'ENVIADO')).rejects.toThrow('"FINALIZADO"');
  });

  it('debería lanzar BadRequestException si el pedido ya está CANCELADO', async () => {
    mockOrderRepository.findById.mockResolvedValue(makeOrder({ statusName: 'CANCELADO' }));

    await expect(useCase.execute('order-1', 'ENVIADO')).rejects.toThrow(BadRequestException);
    await expect(useCase.execute('order-1', 'ENVIADO')).rejects.toThrow('"CANCELADO"');
  });
  ...
  it('CA5 — debería actualizar el estado y registrar en el historial', async () => {
    ...
    expect(mockOrderRepository.updateStatus).toHaveBeenCalledWith('order-1', 'status-enviado');
  ...
  it('CA6 — debería otorgar MoonPoints al cambiar a CONFIRMADO (1 pt por S/2 gastados)', async () => {
    ...
    mockOrderRepository.findById.mockResolvedValue(makeOrder({ statusName: 'EN PROCESO', pointsAwarded: false }));
    mockOrderRepository.getStatusIdByName.mockResolvedValue('status-confirmado');
    mockOrderRepository.updateStatus.mockResolvedValue(undefined);
    mockOrderRepository.addHistory.mockResolvedValue(undefined);
    ...
    expect(mockOrderRepository.markPointsAwarded).toHaveBeenCalledWith('order-1');
    expect(result.pointsAwarded).toBe(50);
  });

  it('CA6 — no debe otorgar MoonPoints si ya fueron acreditados (idempotencia)', async () => {
    mockOrderRepository.findById.mockResolvedValue(makeOrder({ pointsAwarded: true }));
    mockOrderRepository.getStatusIdByName.mockResolvedValue('status-confirmado');
    mockOrderRepository.updateStatus.mockResolvedValue(undefined);
    mockOrderRepository.addHistory.mockResolvedValue(undefined);
